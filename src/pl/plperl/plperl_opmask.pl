...
my @allowed_ops = (
	...
	# Notes:
	# * custom is allowed as it's not deemed a likely security risk as it can't
	# be generated from perl so would only be seen if the DBA had chosen to
	# load a module that used it. Even then it's unlikely to be seen because it's
	# typically generated by compiler plugins that operate after PL_op_mask checks.
);
